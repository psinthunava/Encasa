...
export async function deleteExpense(expenseId: string): Promise<{ error?: string } | void> {
  const member = await requireMember()
  if (member.role !== 'ADMIN') return

  // BillPayment.expenseId has no onDelete cascade (by design — see
  // schema.prisma D12/BillPayment comment): a bill's payment history must
  // stay intact, so an expense that a bill payment generated can't be
  // deleted on its own. Delete the bill (or void the expense) instead.
  const linkedPayment = await prisma.billPayment.findUnique({
    where: { expenseId },
    select: { id: true },
  })
  if (linkedPayment) {
    return {
      error:
        'This expense was generated by a bill payment and can’t be deleted directly. Void it instead, or delete the bill to remove its payment history.',
    }
  }
